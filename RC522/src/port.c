...
/* SPI dynamic blacklist. */
struct gpio_pin_t spi_used_pins[SPI_BASE_PORTCNT + SPI_SS_CNT] ={
{'B', 5},	/* SPI SCK  */
{'B', 4},	/* SPI MISO */
{'B', 5}	/* SPI MOSI */

/* The array afterwards continues on with the client select pins, though
 * intially there are 0
 */
};
...
#ifdef SPI_EN
	for(size_t i = 0; i < (get_spi_portcnt()); i++){
		if( spi_used_pins[i].car == car &&
			spi_used_pins[i].pin == id){
			return true;
		}
	}
#endif /* SPI_EN */
